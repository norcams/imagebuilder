...
if [[ $platform == 'ubuntu' ]] && [[ $major_version == 26 ]]; then
  # Ubuntu 26 requires the .deb installer
  curl -o linux-grid-latest https://iaas-repo.uio.no/nrec/vgpu/nvidia/19.5/ubuntu26/Guest_Drivers/nvidia-linux-grid-580_580.159.03_amd64.deb
...
if sudo grep -q -ir 'bgo-default' /run/cloud-init/; then
    ...
    sudo curl -s https://download.iaas.uio.no/nrec/nrec-resources/files/nvidia-vgpu/bgo-client-token-latest -s -o /etc/nvidia/ClientConfigToken/bgo-client-token-latest
    ...
    sudo mkdir -p /etc/nvidia/ClientConfigToken
    sudo curl -s https://download.iaas.uio.no/nrec/nrec-resources/files/nvidia-vgpu/osl-client-token-latest -s -o /etc/nvidia/ClientConfigToken/osl-client-token-latest
else
